...
class Sprite:
    ...
    def collide(self, other_object):
        if get_distance(self.get_position(), other_object.get_position()) < self.get_radius() + other_object.get_radius():
            return True
        else:
            return False
...
def draw(canvas):
    ...
    if started:
        ...
        process_sprite_group(canvas, rock_group) # draw rock group
...
def rock_spawner():
    ...
    # prevent rock too close to ship
    while(not_proper_pos):
        rock_pos = [random.randrange(0, WIDTH), random.randrange(0, HEIGHT)]
        if get_distance(rock_pos, my_ship.get_position()) >= 120:
            not_proper_pos = False
        else:
            not_proper_pos = True
        
    #    print get_distance(rock_pos, my_ship.get_position()) 
    print get_distance(rock_pos, my_ship.get_position())   
    rock_vel = [random.random() * 1.5 - 0.3, random.random() * 1.5 - 0.3]
